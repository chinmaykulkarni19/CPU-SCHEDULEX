...
class Input {
  constructor() {
    this.pid = [];  //For process id   [This line creates an empty array called pid within the object. It is intended to store process IDs.]
...
document.querySelector(".plusbtn").onclick = () => {
  process = process + 1;  //it increases the value of the variable process by 1.
}
...
function findWaitingTime() {
  ...
  service_time[0] = mainOutput.o_arrivaltime[0];  //This line sets the value of service_time[0] to the arrival time of the first process, as this represents the time when the first process starts executing.
  mainOutput.waitingTime[0] = 0;  //as there's no waiting time for the first process.

  //Time Complexity = O(n) where n = number of process
  // calculating waiting time
  for (var i = 1; i < mainInput.arrivalTime.length; i++) {  //This loop iterates through the processes, starting from the second process (index 1).
